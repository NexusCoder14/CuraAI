...
function todayISO() {
  const d = new Date();
  d.setMinutes(d.getMinutes() - d.getTimezoneOffset()); // normalize to local YYYY-MM-DD
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Compute today/maxDate AFTER mount so SSR HTML (UTC) and client HTML
  // (user's timezone) don't disagree. Until then, render a skeleton.
  const [today, setToday] = useState<string>("");
  const maxDate = useMemo(() => (today ? addDaysISO(today, 90) : ""), [today]);
  ...
  // Quick day chips for the next 7 days
  const quickDays = useMemo(() => (today ? Array.from({ length: 7 }).map((_, i) => addDaysISO(today, i)) : []), [today]);

  // Build slot statuses for the chosen date
  const slotStatuses = useMemo(() => {
    if (!mounted || !date || !today) {
      return SLOTS.map((s) => ({ time: s, taken: false, past: false, disabled: false }));
    }
    const isToday = date === today;
    const nowM = currentTimeHM();
    return SLOTS.map((s) => {
      const taken = isSlotTaken(salon.id, date, s);
      const past = isToday && slotMinutes(s) <= nowM + 15; // 15-min buffer
      return { time: s, taken, past, disabled: taken || past };
    });
  }, [mounted, date, today, salon.id, isSlotTaken, bookings]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pt-10 pb-24">
      <Link href="/salons" className="text-xs text-muted inline-flex items-center gap-1 mb-6 hover:text-ink transition">
        <ArrowLeft className="w-3 h-3" /> Back to Matchmaker
      </Link>

      <AnimatePresence mode="wait">
        {confirmed ? (
          <motion.div key="ok" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="glass-strong rounded-3xl p-10 md:p-14 text-center relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-muted mt-3 text-sm md:text-base">
                <span className="text-ink">{confirmed.service}</span> at <span className="text-ink">{salon.name}</span>
                <br />
                <span className="text-ink">{prettyDate(confirmed.date)}</span> at <span className="text-ink">{confirmed.time}</span>
              </p>
              <div className="inline-flex items-center gap-1.5 chip mt-5">
                <Lock className="w-3 h-3" /> Slot locked
              </div>
              ...
              <div className="mt-7 flex justify-center gap-2 flex-wrap">
                <button onClick={resetForm} className="btn-ghost text-xs">Book another slot</button>
                <Link href="/roadmap" className="btn-ghost text-xs">Updated roadmap</Link>
                <Link href="/chat" className="btn-primary text-xs">Prep with CURA</Link>
              </div>
            </div>
          </motion.div>
        ) : (
          <motion.div key="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-[1.3fr_1fr] gap-5">
            {/* Salon + services */}
                    ...
                    {salon.services.map((s, i) => (
                      <button key={s.name} onClick={() => setServiceIdx(i)}
                        className={`w-full flex items-center justify-between p-3.5 rounded-xl border transition text-left ${
                          serviceIdx === i ? "bg-gradient-to-r from-violet/20 to-cyan/10 border-violet/40 ring-1 ring-violet/30" : "bg-white/[0.03] border-white/10 hover:border-white/20 hover:bg-white/[0.05]"
                        }`}>
                        <div>
            ...
            <div className="glass rounded-3xl p-6">
              ...
              {!mounted ? (
                <div className="space-y-3">
                  <div className="h-16 rounded-xl bg-white/[0.03] border border-white/5 shimmer relative overflow-hidden" />
                  <div className="h-10 rounded-xl bg-white/[0.03] border border-white/5 shimmer relative overflow-hidden" />
                  <div className="grid grid-cols-3 gap-2">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="h-9 rounded-lg bg-white/[0.03] border border-white/5 shimmer relative overflow-hidden" />
                    ))}
                  </div>
                </div>
              ) : (
                  ...
                  return (
                    <button key={d} onClick={() => setDate(d)}
                      className={`flex-shrink-0 w-12 py-2 rounded-xl border text-center transition ${
                        active ? "bg-gradient-to-br from-violet/30 to-cyan/20 border-violet/60 text-ink" : "bg-white/[0.03] border-white/10 text-muted hover:text-ink hover:border-white/20"
                      }`}>
              ...
              {/* Native date picker for anything farther */}
              <div className="mt-4">
                  ...
                  return (
                    <button
                      ...
                      className={`relative py-2.5 rounded-lg text-xs border transition ${
                        s.disabled
                          ? "bg-white/[0.02] border-white/5 text-muted/50 cursor-not-allowed line-through"
                          : selected
                            ? "bg-gradient-to-r from-violet/30 to-cyan/20 border-violet/60 text-ink shadow-[0_0_0_1px_rgba(139,92,246,0.5),0_0_18px_-6px_rgba(139,92,246,0.7)]"
              ...
              {error && (
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="mt-3 text-[11px] text-plasma">
                  {error}
                </motion.div>
              )}
      ...
      {/* Your bookings (compact list) — mounted guard avoids SSR/client mismatch from persisted store */}
      {mounted && bookings.length > 0 && !confirmed && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mt-6 glass rounded-2xl p-4">
