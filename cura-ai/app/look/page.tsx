...
import type { LookBlueprint } from "@/lib/ai/recommendations";
...
export default function Page() {
  ...
  const [look, setLook] = useState<LookBlueprint | null>(null);
    ...
    <div className="max-w-6xl mx-auto px-4 pt-12 pb-24">
      <div className="text-center mb-8">
        ...
        <h1 className="font-display text-4xl md:text-5xl tracking-tight">
          Describe a look. <span className="text-grad">Get the blueprint.</span>
        </h1>
        <p className="text-muted mt-3 max-w-xl mx-auto">
          CURA generates a full aesthetic — hair, skin, makeup, services, and budget — from a single sentence.
        </p>
      </div>
      ...
      <div className="glass-strong rounded-2xl p-4 max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {PROMPTS.map((p) => (
            <button key={p} onClick={() => generate(p)} className="chip hover:scale-105 transition">{p}</button>
          ))}
        </div>
      </div>
      ...
      {look && !loading && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mt-10">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            <Card title="Hair">
              <Row k="Style" v={look.hair.style} />
              <Row k="Color" v={look.hair.color} />
            ...
            <Card title="Makeup">
              <Row k="Eyes" v={look.makeup.eyes} />
              <Row k="Lips" v={look.makeup.lips} />
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="glass rounded-2xl p-6">
      <div className="chip mb-3">{title}</div>
