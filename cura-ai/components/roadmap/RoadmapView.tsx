...
import type { Roadmap, RoadmapTask } from "@/lib/ai/roadmap";
...
const ICON: Record<RoadmapTask["type"], React.ComponentType<{className?: string}>> = {
  salon: Scissors, home: Home, product: ShoppingBag, consult: MessageCircle,
};
    ...
    <div className="max-w-6xl mx-auto px-4 pt-12 pb-24">
      <div className="text-center mb-10">
        ...
        <h1 className="font-display text-4xl md:text-5xl tracking-tight">
          Tell us your <span className="text-grad">goal.</span>
        </h1>
        <p className="text-muted mt-3 max-w-xl mx-auto">
          CURA designs a week-by-week beauty arc — salon visits, home rituals, products, milestones.
        </p>
      </div>
      ...
      <div className="glass-strong rounded-2xl p-4 md:p-5 max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {QUICK_GOALS.map((q) => (
            <button key={q.label} onClick={() => generate(q.goal, q.days)} className="chip hover:scale-105 transition">
              {q.label}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-10 text-center">
      ...
      {/* Roadmap */}
      {roadmap && !loading && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mt-10">
          {/* Headline */}
          <div className="glass-strong rounded-3xl p-6 md:p-8 relative overflow-hidden mb-6">
                          ...
                          <div key={j} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/5 hover:border-white/15 transition">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-[11px] text-muted mt-0.5 flex items-center gap-2">
                                ...
                                <span>·</span>
                                <span className={
                                  t.priority === "high" ? "text-plasma" :
                                  t.priority === "medium" ? "text-cyan" : "text-muted"
                                }>{t.priority}</span>
                              </div>
                            </div>
                          </div>
      ...
      {!roadmap && !loading && (
        <div className="text-center text-muted text-sm mt-16">
          Enter a goal above to generate your first roadmap.
        </div>
      )}
    </div>
  );
