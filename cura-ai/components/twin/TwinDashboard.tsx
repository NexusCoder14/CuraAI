      ...
      <div className="max-w-3xl mx-auto px-4 pt-20 pb-24 text-center">
        <div className="glass rounded-3xl p-12">
          ...
          <p className="text-muted mt-2 max-w-md mx-auto">
            Answer 7 quick questions and CURA will synthesize your persona, style DNA, and beauty score.
          </p>
          <Link href="/twin/onboarding" className="btn-primary mt-6 inline-flex">
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-12 pb-24">
      ...
      <motion.div
        ...
        className="glass-strong rounded-3xl p-8 md:p-10 relative overflow-hidden mb-6"
      >
      ...
      {/* Grid: Style DNA / Palette / Next action */}
      <div className="grid md:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass rounded-2xl p-6 md:col-span-2">
          <div className="flex items-center justify-between mb-5">
            <div>
              <div className="text-xs text-muted">Module</div>
              <h3 className="font-display text-xl">Style DNA</h3>
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {twin.color_palette.map((c) => (
              <div key={c} className="aspect-square rounded-xl relative overflow-hidden ring-1 ring-white/10"
                style={{ background: c }}>
      ...
      {/* Next action card */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-strong rounded-2xl p-6 mt-5 relative overflow-hidden">
        ...
        <div className="relative grid md:grid-cols-[1fr_auto] gap-4 items-center">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/roadmap" className="btn-ghost"><CalendarRange className="w-4 h-4" /> Build Roadmap</Link>
            <Link href="/salons" className="btn-ghost"><Wand2 className="w-4 h-4" /> Match Salons</Link>
            <Link href="/chat" className="btn-primary"><MessageCircle className="w-4 h-4" /> Talk to CURA</Link>
          </div>
        </div>
      </motion.div>
      ...
      {profile && (
        <div className="glass rounded-2xl p-6 mt-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <Field k="Age" v={profile.ageRange} />
            <Field k="Hair" v={profile.hairType} />
            <Field k="Skin" v={profile.skinType} />
            <Field k="Budget" v={profile.budget} />
            <Field k="Goals" v={profile.goals.join(", ")} />
            <Field k="Styles" v={profile.styles.join(", ")} />
            <Field k="Events" v={profile.events.join(", ")} />
            <Field k="Location" v={profile.location ?? "Mumbai"} />
          </div>
        </div>
      )}
    </div>
