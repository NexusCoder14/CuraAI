...
export async function* streamChat(messages, twinContext) {
  if (!hasGroq()) {
...
const TOPICS = [
  ...
  {
    match: /\b(salon|parlou?r|near|bandra|juhu|powai|andheri|bkc|worli|khar|lower parel|colaba|malad|goregaon)\b/i,
    reply: ({ input }) => {
      ...
      return `Here are 3 ${area.charAt(0).toUpperCase() + area.slice(1)} picks aligned with your style:\n\n- ${picks.join("\n- ")}\n\nOpen the **Salon Matchmaker** for compatibility scores against your Beauty Twin?`;
    },
  },
  {
    match: /\b(skincare|skin care|routine|cleanser|moisturi[sz]er|spf|sunscreen|toner|serum)\b/i,
    reply: () =>
  ...
  {
    match: /\b(thanks?|thx|ty)\b/i,
    reply: () => `Anytime. Want me to **save this to your roadmap** or **find a salon** that fits what we discussed?`,
  },
  {
    match: /\b(who are you|what are you|what can you do|help|capabilities)\b/i,
    reply: () =>
      `I'm **CURA**, an AI Beauty Operating System. I can:\n\n- **Chat** through any skin, hair, makeup, or styling question\n- Build your **Beauty Twin** — a living style persona\n- Generate **week-by-week roadmaps** for any goal\n- **Match you** with Mumbai salons based on compatibility (not search)\n- Explore **looks** from one sentence\n- Track **insights** — your beauty score, strengths, trending picks\n\nWhat would you like to start with?`,
  },
];

const GENERIC = [
  (q) => `Good question. Let me think about "${trim(q)}" through your beauty lens.\n\nThe best answers usually combine three things: **what suits your features**, **what fits your lifestyle**, and **what you can maintain weekly**. Tell me a bit more — your hair/skin type, the occasion, or the vibe you're going for — and I'll get specific.`,
  (q) => `Interesting — "${trim(q)}" can go a few directions. Quick clarifiers:\n\n- Is this for an **everyday upgrade** or a **specific event**?\n- What's your **time horizon** — a week, a month, longer?\n- What's your **comfort zone** — minimal, glam, editorial?\n\nAnswer any one and I'll get tactical.`,
  (q) => `Let's break "${trim(q)}" down into something actionable:\n\n- **Today:** one quick win you can do at home\n- **This week:** one salon-grade upgrade\n- **This month:** one transformation milestone\n\nTell me your starting point and I'll fill in the specifics.`,
];
function trim(s, max = 60) { return s.length > max ? s.slice(0, max - 1).trim() + "…" : s; }

function demoReply(messages, twin) {
  const lastUser = [...messages].reverse().find((m) => m.role === "user");
  const input = lastUser?.content ?? "";
  const turn = messages.filter((m) => m.role === "user").length;
  const persona = twin?.persona;
  for (const t of TOPICS) if (t.match.test(input)) return t.reply({ input, turn, persona });
  return GENERIC[(turn - 1 + GENERIC.length) % GENERIC.length](input || "your question");
}
