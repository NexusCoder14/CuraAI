  ...
  return (
    <div aria-hidden className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      ...
      <motion.div
        ...
        style={{ background: "radial-gradient(closest-side, rgba(139,92,246,0.35), transparent 70%)", filter: "blur(60px)" }}
        animate={{ x: [0, 80, -40, 0], y: [0, 40, -20, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-1/3 -right-40 w-[55rem] h-[55rem] rounded-full"
        style={{ background: "radial-gradient(closest-side, rgba(34,211,238,0.28), transparent 70%)", filter: "blur(70px)" }}
        animate={{ x: [0, -60, 30, 0], y: [0, -30, 50, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -bottom-40 left-1/4 w-[50rem] h-[50rem] rounded-full"
        style={{ background: "radial-gradient(closest-side, rgba(232,121,249,0.22), transparent 70%)", filter: "blur(70px)" }}
