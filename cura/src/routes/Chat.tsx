...
export default function Chat() {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setMessages((m) => m.map((msg) => msg.id === assistantId ? { ...msg, content: "Sorry — something interrupted my thinking. Try again." } : msg));
    } finally {
      setStreaming(false);
    }
  }

  function newChat() { if (streaming) return; setMessages([]); setInput(""); taRef.current?.focus(); }
    ...
    <div className="max-w-4xl mx-auto px-4 pt-10 pb-40">
      <div className="text-center mb-8">
        <div className="chip mx-auto mb-3">
          <span className="w-1.5 h-1.5 rounded-full bg-cyan animate-pulse" /> CURA Agent · streaming via Groq
        </div>
      ...
      <div ref={scrollRef} className="glass rounded-3xl p-2 md:p-4 h-[60vh] overflow-y-auto relative">
        {messages.length === 0 ? (
          <div className="p-8 text-center h-full flex flex-col items-center justify-center">
            ...
            <p className="text-muted text-sm max-w-md mx-auto">I'm CURA — your AI Beauty Concierge. Ask me anything about hair, skin, salons, weddings, makeup, or routines.</p>
            <div className="flex flex-wrap gap-2 justify-center mt-6 max-w-2xl">
              {SUGGESTIONS.map((s, i) => (
                <motion.button key={s} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 * i }}
                  onClick={() => send(s)} className="chip hover:scale-105 transition cursor-pointer">{s}</motion.button>
              ))}
            </div>
          </div>
        ) : (
              ...
              {messages.map((m) => (
                <motion.div key={m.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    m.role === "user" ? "bg-gradient-to-br from-violet/30 to-aurora/30 border border-violet/30 text-ink" : "glass text-ink/95"
                  }`}>
      ...
      <form onSubmit={(e) => { e.preventDefault(); send(); }} className="fixed bottom-5 inset-x-0 px-4 z-30">
        <div className="max-w-3xl mx-auto">
          <div className="glass-strong rounded-2xl p-2.5 flex items-end gap-2 ring-glow">
            <button type="button" onClick={newChat} disabled={streaming} title="New chat"
            ...
            <button type="submit" disabled={!input.trim() || streaming} title="Send"
          ...
          <div className="text-[10px] text-muted text-center mt-2">CURA can make mistakes. Verify with your stylist for medical or allergy guidance.</div>
        </div>
      </form>
    </div>
  );
      ...
      {blocks.map((block, bi) => {
        ...
        return (
          <p key={bi} className="leading-relaxed">
            {lines.map((l, i) => (
              <span key={i}>{renderInline(l)}{i < lines.length - 1 && <br />}</span>
            ))}
          </p>
        );
      })}
