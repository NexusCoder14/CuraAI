...
import { useCura, type TwinProfile } from "@/lib/store/useCura";

type Step = { key: keyof TwinProfile; title: string; subtitle: string; type: "single"|"multi"; options: string[] };
...
const STEPS: Step[] = [
  ...
  { key: "goals", title: "What are your beauty goals?", subtitle: "Pick up to four.", type: "multi", options: ["Glow up", "Hair transformation", "Anti-aging", "Bridal prep", "Daily routine", "Confidence boost", "Event ready", "Hair health"] },
  ...
  { key: "budget", title: "Comfortable monthly beauty budget?", subtitle: "Tunes service and salon recommendations.", type: "single", options: ["Under ₹3K", "₹3–8K", "₹8–15K", "₹15–30K", "₹30K+"] },
...
export default function Onboarding() {
  ...
  const isSelected = (v: string) => current.type === "single" ? values[current.key] === v : Array.isArray(values[current.key]) && (values[current.key] as string[]).includes(v);
  const canNext = current.type === "single" ? Boolean(value) : Array.isArray(value) && (value as string[]).length > 0;
  ...
  async function finish() {
    ...
    setProfile(profile);
    try {
      const res = await fetch("/api/twin", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(profile) });
    ...
    <div className="max-w-3xl mx-auto px-4 pt-14 pb-24">
      <div className="mb-8">
        ...
        <div className="h-1 rounded-full bg-white/5 overflow-hidden">
          <motion.div className="h-full bg-gradient-to-r from-violet via-aurora to-cyan"
            animate={{ width: `${progress}%` }} transition={{ type: "spring", stiffness: 80, damping: 20 }} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
            transition={{ duration: 0.3 }} className="glass rounded-3xl p-8 md:p-10">
                ...
                return (
                  <button key={opt} onClick={() => setVal(opt)}
                    className={`relative px-4 py-2.5 rounded-xl text-sm border transition ${
                      selected ? "bg-gradient-to-br from-violet/30 to-aurora/20 border-violet/60 text-ink"
            ...
            <div className="flex items-center justify-between mt-10">
              <button onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0}
                className="btn-ghost disabled:opacity-30 disabled:cursor-not-allowed"><ArrowLeft className="w-4 h-4" /> Back</button>
              {step === STEPS.length - 1 ? (
                ...
                <button onClick={() => setStep((s) => Math.min(STEPS.length - 1, s + 1))} disabled={!canNext} className="btn-primary disabled:opacity-50">
